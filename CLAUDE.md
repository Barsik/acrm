...
- When the user writes "сохрани", first verify that `npm run build` succeeds; only if the build passes, stage new files (`git add`), commit, and push to the current branch. If the build fails, do not commit or push — fix the errors first.
